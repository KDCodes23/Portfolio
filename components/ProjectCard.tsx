...
export function ProjectCard({ project, compact = false }: ProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className={compact ? "mb-4 flex items-start justify-between gap-3" : "mb-5 flex items-start justify-between gap-4"}>
      ...
      {!compact && (
        <div className="relative mb-5 h-44 overflow-hidden border border-white/12 bg-gradient-to-br from-panel to-bg sm:h-48">
          <motion.div
            ...
            transition={{
              duration: MOTION_SECONDS.medium - 0.08,
              ease: EASE_OUT_QUAD,
            }}
          />
      ...
      <h3
        className={
          compact
            ? "font-display text-[1.25rem] font-medium leading-tight text-text"
            : "font-display text-[1.6rem] font-medium leading-tight text-text"
        }
      >
        {project.title}
      </h3>
      ...
      <p
        className={
          compact
            ? "mt-3 line-clamp-3 text-sm leading-relaxed text-muted"
      ...
      <ul className={compact ? "mt-4 flex flex-wrap gap-2" : "mt-5 flex flex-wrap gap-2"}>
        {(compact ? project.tech.slice(0, 3) : project.tech).map((tech) => (
          <li
            key={tech}
            className="rounded-full border border-white/12 bg-panel px-2.5 py-1 text-xs text-text"
          >
            {tech}
          </li>
        ))}
      ...
      <div className={compact ? "mt-auto flex flex-wrap gap-2 pt-5" : "mt-auto flex flex-wrap gap-2 pt-7"}>
        {project.links.demo ? (
          <a
        ...
        {!compact && project.links.caseStudy ? (
          <a
            href={project.links.caseStudy}
